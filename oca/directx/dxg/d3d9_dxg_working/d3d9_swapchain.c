...
static HRESULT WINAPI Direct3DSwapChain9_QueryInterface(LPDIRECT3DSWAPCHAIN9EX iface, REFIID riid, void** ppvObject)
{
    ...
    // {
        // Direct3DSwapChain9_AddRef(iface);
        // *ppvObject = &This->lpVtbl;
        // return S_OK;
    // }	

    // *ppvObject = NULL;
    return IDirect3DSwapChain9_QueryInterface(SwapChainProxyInterface, riid, ppvObject);
}

static ULONG WINAPI Direct3DSwapChain9_Release(LPDIRECT3DSWAPCHAIN9EX iface)
{
    return IDirect3DSwapChain9_Release(SwapChainProxyInterface);
}
...
{
    return IDirect3DSwapChain9_Present(SwapChainProxyInterface, pSourceRect, pDestRect, hDestWindowOverride, pDirtyRegion, dwFlags);
}

static HRESULT WINAPI Direct3DSwapChain9_GetFrontBufferData(LPDIRECT3DSWAPCHAIN9EX iface, IDirect3DSurface9* pDestSurface)
{
    return IDirect3DSwapChain9_GetFrontBufferData(SwapChainProxyInterface, pDestSurface);
}
...
{
    return IDirect3DSwapChain9_GetBackBuffer(SwapChainProxyInterface, iBackBuffer, Type, ppBackBuffer);
}

static HRESULT WINAPI Direct3DSwapChain9_GetRasterStatus(LPDIRECT3DSWAPCHAIN9EX iface, D3DRASTER_STATUS* pRasterStatus)
{
    return IDirect3DSwapChain9_GetRasterStatus(SwapChainProxyInterface, pRasterStatus);
}

static HRESULT WINAPI Direct3DSwapChain9_GetDisplayMode(LPDIRECT3DSWAPCHAIN9EX iface, D3DDISPLAYMODE* pMode)
{
    return IDirect3DSwapChain9_GetDisplayMode(SwapChainProxyInterface, pMode);
}
...
static HRESULT WINAPI Direct3DSwapChain9_GetDevice(LPDIRECT3DSWAPCHAIN9EX iface, IDirect3DDevice9** ppDevice)
{
    ...
    // UNLOCK_D3DDEVICE9();
    // return D3D_OK;
	return IDirect3DSwapChain9_GetDevice(SwapChainProxyInterface, ppDevice);	
}
...
static HRESULT WINAPI Direct3DSwapChain9_GetPresentParameters(LPDIRECT3DSWAPCHAIN9EX iface, D3DPRESENT_PARAMETERS* pPresentationParameters)
{
    ...
    // UNLOCK_D3DDEVICE9();
    // return D3D_OK;
	return IDirect3DSwapChain9_GetPresentParameters(SwapChainProxyInterface, pPresentationParameters);
}
        ...
        D3DDISPLAYMODEEX *mode, D3DDISPLAYROTATION *rotation)
{
    ...
    hr = Direct3DSwapChain9_GetDisplayMode(iface, &pMode);
    if (SUCCEEDED(hr)) {       
