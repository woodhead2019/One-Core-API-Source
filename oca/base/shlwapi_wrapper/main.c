...
HRESULT GetModuleResourceData(HMODULE hModule, LPCWSTR lpName, LPCWSTR lpType, LPVOID *lpRes, DWORD *dwSize)
{
  HRSRC ResourceW; // eax
  HRSRC hResInfo; // eax
  DWORD ResourceSize; // edi
  HGLOBAL Resource; // eax
  LPVOID ResourceData; // eax

  ResourceW = FindResourceW(hModule, lpName, lpType);
  hResInfo = ResourceW;
  if ( !ResourceW )
    return HRESULT_FROM_WIN32(GetLastError());
  ResourceSize = SizeofResource(hModule, ResourceW);
  if ( !ResourceSize )
    return HRESULT_FROM_WIN32(GetLastError());
...
HRESULT WINAPI CreateStreamSTOnModuleResource(HMODULE hModule, BYTE *pInit, const WCHAR *cbInit, IStream **a4)
{
  ...
  ModuleResourceData = GetModuleResourceData(hModule, (LPCWSTR)pInit, cbInit, (LPVOID*)&pInit, (DWORD*)&cbInit);
